...
While LaMa handles the "disappearing act," **Segment Anything** acts as the magician's "assistant", which identifies the object to be removed.

**Image Encoder & Mask Decoder:**
SAM uses a heavy **Vision Transformer (ViT)** backbone to extract image features. These features are precomputed (encoded) only once per image to ensure real-time responsiveness.
